...
export const getAllOficceAndCodeCity = async()=>{
    let res = await fetch("http://172.16.101.146:5687/offices?city")
...
export const getAllOficceCityAndMovil = async()=>{
    let res = await fetch("http://172.16.101.146:5687/offices?country=España")
    let data = await res.json();
    let dataUpdate = data.map(val =>{
        return {
            code_office: val.code_office,
...
export const getAllOffices = async(codeOffice)=>{
    let res = await fetch(`http://172.16.101.146:5687/offices?code_office=${codeOffice}`)
    let data = await res.json();
    return data;
}
...
export const AllDirectionsWithClientsInFuenlabrada = async()=>{
    let clients = await fetch("http://172.16.101.146:5684/clients?city=Fuenlabrada").then(response => response.json())
